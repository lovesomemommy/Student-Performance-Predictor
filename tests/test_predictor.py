...
def test_prepare_features():
    df = pd.DataFrame({
        'grade1': [80], 'grade2': [70], 'grade3': [75],
        'attended_lectures': [10], 'total_lectures': [20],
        'late_assignments': [2], 'total_assignments': [5],
        'at_risk': [0]
    })
...
def test_predict_risk():
    ...
    model = train_model(df)
    student = {
        'grade1': 40, 'grade2': 45, 'grade3': 50,
        'attended_lectures': 5, 'total_lectures': 20,
        'late_assignments': 4, 'total_assignments': 5
    }
...
def test_low_risk_student():
    ...
    model = train_model(df)
    student = {
        'grade1': 90, 'grade2': 92, 'grade3': 88,
        'attended_lectures': 19, 'total_lectures': 20,
        'late_assignments': 0, 'total_assignments': 5
    }
    is_at_risk, prob = predict_risk(model, student)
...
def test_high_risk_student():
    ...
    model = train_model(df)
    student = {
        'grade1': 30, 'grade2': 35, 'grade3': 40,
        'attended_lectures': 4, 'total_lectures': 20,
        'late_assignments': 5, 'total_assignments': 5
    }
    is_at_risk, prob = predict_risk(model, student)
    assert is_at_risk